...
where $df(t)$ is the number of catalog entries containing token $t$. This is precomputed once at startup from the full corpus.

**Edge case**: If a token appears in zero catalog entries ($df = 0$), the formula yields $\frac{1}{\log 2} \approx 1.44$. Novel tokens receive high weight, which is intentional — rare user terms should strongly discriminate when they do match.
